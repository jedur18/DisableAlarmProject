...
//    let mut exti  = Exti::new(dp.EXTI);
    ...
    // Asign pins for serial communication
    ...
    // Serial values for communication
    let serial_alarm_on: u8 = 0b01; // We receive a 1 to say that we should switch on the alarm
    let serial_alarm_off: u8 = 0b01; // we send a 1 to say that pin has been correctly introduced in order to disable the alarm
...
let correct_pin:[u8;4] = [4,5,7,3];
    ...
    //    let i: u8;
        ...
        if n_correct_digits == 4 { // Correct pin
            ...
            block!(tx.write(serial_alarm_off)).ok(); // we send to the serial signal of alarm disabled
        }
        else { // Incorrect pin
            introduced_pin = [48u8; 4]; // Reset the introduced pin
            n_introduced = 0; // Reset the counter of introduced digits
       ...
       if alarm_state == false {
        ...
        // The activation of the alarm will be based on a signal received by serial port
        // Read serial port    
        let mut received = block!(rx.read()).unwrap();
        if received == serial_alarm_on {
